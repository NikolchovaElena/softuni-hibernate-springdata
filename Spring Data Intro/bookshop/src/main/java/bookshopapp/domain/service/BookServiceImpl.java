...
import bookshopapp.domain.entities.*;
import bookshopapp.domain.repository.AuthorRepository;
import bookshopapp.domain.repository.BookRepository;
...
public class BookServiceImpl implements BookService {
    ...
    private static final String BOOK_FILE_PATH = "C:\\Users\\elena n\\Desktop\\isstr\\bookshop\\src\\main\\resources\\files\\books.txt";
    ...
    @Override
    public Set<String> getAllAuthorsWithBooksBefore() {
        List<Book> books = this.bookRepository.findAllByReleaseDateBefore(LocalDate.parse("1990-01-01"));

        return books.stream().map(b ->
                String.format("%s %s", b.getAuthor().getFirstName(), b.getAuthor().getLastName()))
    ...
    public List<String> getAllBooksByAuthorName() {
        List<Book> books = this.bookRepository.findAllByAuthor_FirstNameAndAuthor_LastName("George", "Powell");

        return books.stream().map(b ->
                String.format("%s %s %s", b.getTitle(), b.getReleaseDate(),b.getCopies()))
    ...
    private Author getRandomAuthor() {
        ...
        int randomId = 0;
        while (randomId == 0) {
            randomId = random.nextInt((int) (this.authorRepository.count() + 1));
        }
    ...
    private Category getRandomCategory() {
        ...
        int randomId = 0;
        while (randomId == 0) {
            randomId = random.nextInt((int) this.categoryRepository.count() + 1);
        }
